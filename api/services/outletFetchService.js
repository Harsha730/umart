      ...
      if (locale == "te_IN") {
        ...
        temp_vendor.findOne({
          ...
          if (null != vendor) {
            ...
            var is_active = false, vendor_plan = vendor.vendor_plan;
            if (null != vendor_plan)
              is_active = vendor_plan.is_active;
            if (null == vendor_plan || (true == is_active && 0 >= dateDiff))
              is_active_vendor = true
            else if (null != vendor_plan && (false == vendor_plan.is_active || 0 < dateDiff)) {
              is_active_vendor = false
              console.log("vendor_plan::" + vendor_plan.is_active)
            }
            //  console.log(JSON.parse(JSON.stringify(vendor.price_book)));
            var opened = "Hours of Operation: ";
            var opened_weeks = [], closed_weeks = [], speacial_weeks = [];
            ...
            if (is_active_vendor) {
              ...
              if (resObj.calender_summary.regular_hours.includes("undefined"))
                resObj.calender_summary.regular_hours = "";
              resolve((resObj));
            }
            else {
              logger.error("Vendor Plan Expired");
              reject(res.status(403).json("Your subscription plan is not activated or expired. Please activate/renew you plan to login and activate your outlet. Thank you for your business."));
            }
          }
          else {
            /*   }
