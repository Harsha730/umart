...
exports.restore_product= function (sku,vendor_id,response) {

   logger.info('Entered into the restore_product service');
   ...
   return new Promise(function (resolve, reject) {
      ...
      try {
         ...
         db.products.findOne({
            ...
            if (null != product) {
               logger.info("Found the product to re-store");
               // Passing the single JSON object with the updated fields. Here keys are DB columns and values are data to be updated.
               product.update({is_deleted: false}).then(status=>{
                  resolve('Product has been re-stored successfully!');
               ...
               reject(response.status(404).send("Unable to re-store the product as no data found. Thank you for your business."));
            }
         }).catch(error => {
            logger.error("Error occured while deleting the vendor ::" + error);
            reject(response.status(500).send("Internal Server Error! Unable to re-store the product. Thank you for your business."));
         });
      }
      catch (err) {
